...
def super_safe_release_date(release)
...
def safe_release_date(release)
	return super_safe_release_date(release)
rescue ArgumentError
	return Time.strptime(release, "%B %d, %Y").strftime "%Y-%m-%d"
end
...
doc = Nokogiri::XML open("http://thegamesdb.net/api/GetPlatformGames.php?platform=3")
print <<-endofgame
