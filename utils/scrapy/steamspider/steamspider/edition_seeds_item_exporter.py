...
class EditionSeedsItemExporter(BaseItemExporter):
    ...
    def export_item(self, item):
        ...
        if("expansion" in itemdict and itemdict['expansion']):
            self.file.write("""
            edition = nil
            GamesSearch.new(query: "%s").search.only(:id).load.each { |g|
                if g.class.name == "Edition"
                    if g.title == "%s"
                        edition = g
                    end
                end
            ...
            if edition
                expansion = Expansion.create(:edition_id => edition.id, :description => "%s", :release_date => "%s", :title => "%s", :coverart_remote_url => '%s')
            end
""" % (itemdict['parent_title'].replace('&amp;','&').replace('"', r'\"'), itemdict['parent_title'].replace('&amp;','&').replace('"', r'\"'), itemdict['description'].replace('\r','\n').replace('&amp;','&').replace('\\', '\\\\').replace('"', r'\"').replace('\t', '').strip(), release_date, itemdict['title'].replace('&amp;','&').replace('"', r'\"'), itemdict['image_url']))
        else:
